...
sub mainLoop_initialized {
	...
	if ($net->getState() == Network::IN_GAME) {
        ...
        my $zeny;
        for (my $i = length($char->{zeny}); $i > 0; $i--) {
            $zeny .= "." if ($i % 3 == 0 && $i!= length($char->{zeny}));
            $zeny .= substr($char->{zeny},length($char->{zeny})-$i,1);
        }
        $title = TF("%s B%s %s Z:%s p%s %s %s",
			$charName, $char->{lv}, $basePercent . '%', $zeny, $weight, $pos, $aiSeq);
